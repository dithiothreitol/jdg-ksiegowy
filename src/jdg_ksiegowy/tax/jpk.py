...
def generate_jpk_v7m(
    ...
    # Oblicz sumy
    total_net = sum(inv.total_net for inv in invoices)
    total_vat = sum(inv.total_vat for inv in invoices)
    ...
    # P_49 — Podstawa do zapłaty (bez odliczeń VAT naliczonego dla ryczałtowca)
    etree.SubElement(poz, "{%s}P_49" % TNS).text = f"{total_vat:.2f}"
    # P_51 — Kwota do wpłaty
    etree.SubElement(poz, "{%s}P_51" % TNS).text = f"{total_vat:.2f}"

    pouczenia = etree.SubElement(deklaracja, "{%s}Pouczenia" % TNS).text = "1"
    ...
    for inv in invoices:
        ...
        etree.SubElement(wiersz, "{%s}LpSprzedazy" % TNS).text = str(invoices.index(inv) + 1)
        ...
        # K_19 — Podstawa opodatkowania stawka 23%
        etree.SubElement(wiersz, "{%s}K_19" % TNS).text = f"{inv.total_net:.2f}"
        # K_20 — VAT stawka 23%
        etree.SubElement(wiersz, "{%s}K_20" % TNS).text = f"{inv.total_vat:.2f}"
    ...
    # Brak zakupów (ryczałt — nie odlicza VAT naliczonego, ale sekcja wymagana)
    ctrl_zak = etree.SubElement(ewidencja, "{%s}ZakupCtrl" % TNS)
    etree.SubElement(ctrl_zak, "{%s}LiczbaWierszyZakupow" % TNS).text = "0"
    etree.SubElement(ctrl_zak, "{%s}PodatekNaliczony" % TNS).text = "0.00"
...
def save_jpk_v7m(
    ...
    xml = generate_jpk_v7m(invoices, month, year, correction)
